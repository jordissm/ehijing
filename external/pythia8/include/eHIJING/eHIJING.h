...
// alphas*PhiG
double alphas_PhiG(double x, double q2, double Qs2, double powerG, double lambdaG, double K);
...
private:
  ...
  double Qs2_self_consistent_eq(double Qs2, double TA, double xB, double Q2);
  double compute_Qs2(double TA, double xB, double Q2);
public:
  MultipleCollision(double K, double pG, double lG);
  // Tabulate Qs table
  void Tabulate(std::filesystem::path table_path);
  double Qs2(double TA, double xB, double Q2) {
      return Qs2Table.interpolate({TA, std::log(xB), std::log(Q2)});
  };
  // compute the qhat of a gluon
  double qhatA(double TA, double xB, double Q2) {
      return rho0*Qs2(TA, xB, Q2)/TA;
  };
  // compute the qhat of a quark
  double qhatF(double TA, double xB, double Q2) {
...
public:
    // constructor, given mode, the K factor, powerG, lambdaG
    eHIJING(int mode, double K, double pG, double lG);
