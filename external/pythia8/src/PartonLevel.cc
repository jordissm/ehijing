...
bool PartonLevel::init( Info* infoPtrIn, Settings& settings,
  ...
  if (doSD || doDD || doSQ || (doHardDiff && beamBPtr->getGammaMode() < 2)) {
  ...
  if (doSD || doDD || doSQ || (doHardDiff && beamAPtr->getGammaMode() < 2)) {
...
bool PartonLevel::next( Event& process, Event& event) {
  ...
  if (doHardDiff) {
    ...
    // wrt. photon beam.
    if ( beamBPtr->isHadron() || ((beamBisGamma || beamBhasGamma)
         && beamBPtr->getGammaMode() == 1)) {
    ...
    if ( beamAPtr->isHadron() || ((beamAisGamma || beamAhasGamma)
         && beamAPtr->getGammaMode() == 1)) {
  ...
  for (int iTry = 0; iTry < NTRY; ++ iTry) {
    ...
    // WK: Here hard system is dump into event
    ...
    // WK: non-interleaved FSR, after process and ISR and MPI
    if (doFSRafterProcess && (nBranchMax <= 0 || nBranch < nBranchMax) ) {
      ...
      for (int i = 0; i < event.size(); ++i)
        if (event[i].isFinal() && event[i].scale() > pTmax)
          pTmax = event[i].scale();
      pTsaveFSR = pTmax;
      //std::cout<< "pTFSR:new = " << pTmax << std::endl;
...
void PartonLevel::setupHardSys(Event& process, Event& event) {
  ...
  if(eHIJING){
    ...
    event.setRy(ry);
    event.setRz(rz);
    event.setSeparationScale(EHIJING::mu2);
  }
  // WK <<<
...
bool PartonLevel::setupResolvedLeptonGamma( Event& process) {
  ...
  // Done for direct-direct processes since no need to reassign beams.
  if ( gammaModeEvent == 4) return true;
...
void PartonLevel::leaveResolvedLeptonGamma( Event& process, Event& event,
  ...
  for (int i = 5; i < event.size(); ++i) {
    if ( (i != iSkipGamma) && (i != iSkipHardDiff) )
  ...
  // Done for direct-direct processes.
  if ( gammaModeEvent == 4) return;
