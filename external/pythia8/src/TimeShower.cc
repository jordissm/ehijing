...
double TimeShower::pTnext( Event& event, double pTbegAll, double pTendAll,
  ...
  //std::cout << "How much diple "<<int(dipEnd.size())<<std::endl;
  for (int iDip = 0; iDip < int(dipEnd.size()); ++iDip) {
    TimeDipoleEnd& dip = dipEnd[iDip];
    dip.pAccept        = 1.0;
    ...
    //std::cout << "dip col type:" << dip.colType << std::endl;
...
void TimeShower::pT2nextQCD(double pT2begDip, double pT2sel,
  ...
  do {
    ...
    // this is in the CoM frame of e+P
    // compute the velocity in the nuclear rest frame
    auto pinA = p.p();
    pinA.bstback(pProton);
    double vx = pinA.px()/pinA.e(), vy = pinA.py()/pinA.e(), vz = pinA.pz()/pinA.e();
    double L = eHIJING_Geometry->compute_L(event.Rx(), event.Ry(), event.Rz(), vx, vy, vz);
    double Thickness = eHIJING_Geometry->compute_TA(event.Rx(), event.Ry(), event.Rz(), vx, vy, vz);
    double Qs2 = L*EHIJING::rho0/Thickness * eHIJING_Gen->Qs2(Thickness, xB, Q20);
    double pT2min_in_A = pT2min;//std::max(pT2min, eHIJING_Gen->Qs2(EHIJING::rho0*L, xB, Q20) );
				
    event.setSeparationScale(Qs2);
    ...
    // Ditto for first-order alpha_strong.
      ...
      if (!eHIJING || dip.pT2 < Qs2 || true) {
        ...
        bool status =  eHIJING_Gen->next_kt2_stochastic(
                        dip.pT2, p.idAbs(),
                        std::max(pinA.e(),0.5), pT2min_in_A,
    ...
    // Abort evolution if below cutoff scale, or below another branching.
        ...
        if (dip.flavour == 21) {
          if (!eHIJING || dip.pT2 < Qs2 || true){
            ...
            double zg;
            eHIJING_weight = eHIJING_Gen->sample_z_stochastic(zg, p.idAbs(), std::max(pinA.e(),.5),
        ...
        if (dip.z > zMin && dip.z < 1. - zMin
            ...
            if (dip.MEtype > 0) {
               ...
               // z weight for g -> g g; optional suppression
               // for massive recoiler.
            } else if (dip.flavour == 21) {
            ...
            wt /= overFac;
            //std::cout << "rad step 2 " << dip.z << " " << dip.pT2 <<  " " << wt << std::endl;
            // Suppression factors for dipole to beam remnant.
            /*if (dip.isrType != 0 && useLocalRecoilNow) {
                std:: cout << "Do not use local recoil for eHIJING" << std::endl;
...
bool TimeShower::branch( Event& event, bool isInterleaved) {
  ...
  // WK >>> For kT-ordered shower, the production vertex is set at the hard vertex
  rad.setRx(event.Rx());
  rad.setRy(event.Ry());
  rad.setRz(event.Rz());
  emt.setRx(event.Rx());
